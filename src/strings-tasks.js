 ...
 */
function getStringLength(value = null) {
 ...
 */
function removeFirstOccurrences(str, value, from = 'indexOf') {
  return str[from](value) === -1
    ? str
    : `${str.slice(0, str[from](value))}${str.slice(
        str[from](value) + value.length
      )}`;
}
 ...
 */
function removeLastOccurrences(...dat) {
  return removeFirstOccurrences(...dat, 'lastIndexOf');
}
...
function sumOfCodes(str = '') {
  ...
  for (let i = 0; i <= str.length; i += 1) {
    acc += str.charCodeAt(i) ? str.charCodeAt(i) : 0;
  }
  return acc;
}
...
function countVowels(str) {
  const temp = str.match(/[aeiuoy]/gi);
...
function isPalindrome(str) {
  const sanitizeStr = str.toLocaleLowerCase().match(/[a-z]/g).join('');
  const part =
    sanitizeStr.length % 2 === 0
      ? sanitizeStr
      : sanitizeStr.slice(0, Math.floor(sanitizeStr.length / 2)) +
        sanitizeStr.slice(Math.ceil(sanitizeStr.length / 2));
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    result +=
      str[i][str[i] === str[i].toUpperCase() ? 'toLowerCase' : 'toUpperCase']();
  }
  return result;
}
...
function encodeToRot13(str) {
  const f = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split('');
  const t = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');
  let output = '';
  const ar = str.split('');
  ar.forEach((letter) => {
    const i = f.findIndex((findArrLetter) => findArrLetter === letter);
    output += i === -1 ? letter : t[i];
  });
  return output;
}
